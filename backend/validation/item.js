...
const createItemVal = [
  check("categoryID").isLength({ min: 1 }).withMessage(`Category is required`),
  check("name").isLength({ min: 1 }).withMessage("Item name is required"),
  ...
  check("discountAmount").custom((value, { req }) => {
    if (req.body.discount === true) {
      if (value === undefined || value === null || value === "") {
        throw new Error(
          "Discount amount is required when discount is selected"
        );
      }
      if (isNaN(value)) {
  ...
  check("status").isBoolean().withMessage("Status is required"),
  check("description")
    .optional()
    .isString()
